...
@Component
public class WorkNoteThirdLevel implements TriggerAction {
    private final Sendler sendler;
    private final Properties pattern = new Properties();


    public WorkNoteThirdLevel(Sendler sendler) {
    ...
    public void execute(Update update) {
        String link = pattern.getProperty("work.note.level.3.link");
        sendler.sendLink(update.getMessage().getChatId(), "Рабочая тетрадь 3 уровня", link);
    }
}