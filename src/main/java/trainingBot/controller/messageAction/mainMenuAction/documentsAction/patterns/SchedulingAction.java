...
@Component
public class SchedulingAction implements TriggerAction {
    private final Sendler sendler;
    private final Properties pattern = new Properties();


    public SchedulingAction(Sendler sendler) {
    ...
    public void execute(Update update) {
        String link = pattern.getProperty("patterns.scheduling");
        sendler.sendLink(update.getMessage().getChatId(), "РСП для ЗП", link);
    }
}