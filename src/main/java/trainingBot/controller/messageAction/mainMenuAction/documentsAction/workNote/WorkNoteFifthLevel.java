...
@Component
public class WorkNoteFifthLevel implements TriggerAction {
    private final Sendler sendler;
    private final Properties pattern = new Properties();


    public WorkNoteFifthLevel(Sendler sendler) {
    ...
    public void execute(Update update) {
        String link = pattern.getProperty("work.note.level.5.link");
        sendler.sendLink(update.getMessage().getChatId(), "Рабочая тетрадь 5 уровня", link);
    }
}
